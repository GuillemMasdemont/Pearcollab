...
// ── Cursor decorations ─────────────────────────────────────────────────────────
function createDecos(displayName: string, color: string) {
...
function handleSidecarMessage(msg: any) {
  ...
  switch (method) {
    case 'peer.connected': {
      ...
      const { cursor, selection } = createDecos(params.displayName, color);
...
// ── Session lifecycle ──────────────────────────────────────────────────────────
async function beginSession(roomName: string, displayName: string, context: vscode.ExtensionContext) {
  if (session) {
  ...
  // Initialize ydocs for all currently open text editors
  for (const editor of vscode.window.visibleTextEditors) {
    const filePath = getRelativePath(editor.document.uri);
    if (!filePath || isFileExcluded(filePath)) continue;
    getOrCreateDoc(filePath, editor.document.getText());
  }
